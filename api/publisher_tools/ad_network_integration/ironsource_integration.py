# api/publisher_tools/ad_network_integration/ironsource_integration.py
"""IronSource integration for Publisher Tools."""
...
class IronSourceConfig(TimeStampedModel):
    """IronSource publisher configuration."""
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name="publisher_tools_ironsource_tenant",
        db_index=True,
    )
    publisher = models.ForeignKey(
        "publisher_tools.Publisher",
        on_delete=models.CASCADE,
        related_name="ironsource_configs",
    )
    ...
    class Meta:
        db_table = "publisher_tools_ironsource_configs"
        verbose_name = _("IronSource Config")
        unique_together = [["publisher"]]

    def __str__(self):
        return f"IronSource: {self.publisher.publisher_id}"

    def sync_revenue(self) -> bool:
        """Revenue data sync করে। Production-এ IronSource API call করো।"""
        import logging
        logging.getLogger(__name__).info(
            f"Syncing IronSource revenue for {self.publisher.publisher_id}"
        )
    ...
    def get_ad_tag(self, ad_unit_id: str) -> str:
        ...
        return f"<!-- IronSource Ad Tag — {self.publisher.publisher_id} — {ad_unit_id} -->"

    def validate_credentials(self) -> bool:
        """API credentials validate করে।"""
        required = self.app_key, self.instance_id
        return all(bool(getattr(self, p, None)) for p in ['app_key', 'instance_id'])
