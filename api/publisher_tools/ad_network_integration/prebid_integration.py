# api/publisher_tools/ad_network_integration/prebid_integration.py
"""Prebid integration for Publisher Tools."""
...
class PrebidConfig(TimeStampedModel):
    """Prebid publisher configuration."""
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.SET_NULL,
        null=True, blank=True,
        related_name="publisher_tools_prebid_tenant",
        db_index=True,
    )
    publisher = models.ForeignKey(
        "publisher_tools.Publisher",
        on_delete=models.CASCADE,
        related_name="prebid_configs",
    )
    is_enabled   = models.BooleanField(default=True, db_index=True)
    is_test_mode = models.BooleanField(default=False)
    bidder = models.CharField(max_length=200, blank=True)
    ...
    def __str__(self):
        return f"Prebid: {self.publisher.publisher_id}"

    def sync_revenue(self) -> bool:
        """Revenue data sync করে। Production-এ Prebid API call করো।"""
        import logging
        logging.getLogger(__name__).info(
            f"Syncing Prebid revenue for {self.publisher.publisher_id}"
        )
    ...
    def get_ad_tag(self, ad_unit_id: str) -> str:
        ...
        return f"<!-- Prebid Ad Tag — {self.publisher.publisher_id} — {ad_unit_id} -->"

    def validate_credentials(self) -> bool:
        """API credentials validate করে।"""
        required = self.bidder, self.params
        return all(bool(getattr(self, p, None)) for p in ['bidder', 'params'])
