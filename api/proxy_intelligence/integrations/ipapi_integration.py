"""
IpApi Integration — IP intelligence via ipapi API.
API key from: IntegrationCredential model -> settings.IPAPI_API_KEY -> os.environ["IPAPI_API_KEY"]
"""
...
class IpApiIntegration:
    ...
    def _resolve_key(self) -> str:
        ...
        return getattr(settings, 'IPAPI_API_KEY', None) or os.environ.get('IPAPI_API_KEY', '')
    ...
    def check(self, ip_address: str) -> dict:
        cache_key = f"pi:ipapi:{ip_address}"
...
FETCH_PLACEHOLDER
    def _fetch(self, ip_address: str) -> dict:
        try:
            url = f"https://ipapi.co/{ip_address}/json/?key={self.api_key}" if self.api_key else f"https://ipapi.co/{ip_address}/json/"
            resp = requests.get(url, timeout=8, headers={"User-Agent": "ProxyIntelligence/1.0"})
            if resp.status_code == 429:
                return {"ip_address": ip_address, "error": "rate_limited", "source": self.SERVICE_NAME}
            resp.raise_for_status()
            d = resp.json()
            if d.get("error"):
                return {"ip_address": ip_address, "error": d.get("reason", "api_error"), "source": self.SERVICE_NAME}
            return {
                ...
                "asn": d.get("asn", ""), "currency": d.get("currency", ""),
                "is_eu": bool(d.get("in_eu", False)),
            }
        except Exception as e:
