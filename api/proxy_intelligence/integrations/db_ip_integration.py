"""
DbIp Integration — IP intelligence via db_ip API.
API key from: IntegrationCredential model -> settings.DB_IP_API_KEY -> os.environ["DB_IP_API_KEY"]
"""
...
class DbIpIntegration:
    ...
    def _resolve_key(self) -> str:
        ...
        return getattr(settings, 'DB_IP_API_KEY', None) or os.environ.get('DB_IP_API_KEY', '')
    ...
    def check(self, ip_address: str) -> dict:
        cache_key = f"pi:db_ip:{ip_address}"
...
FETCH_PLACEHOLDER
    def _fetch(self, ip_address: str) -> dict:
        try:
            url = f"https://api.db-ip.com/v2/{self.api_key}/{ip_address}" if self.api_key else f"https://api.db-ip.com/v2/free/{ip_address}"
            ...
            d = resp.json()
            if d.get("error"):
                return {"ip_address": ip_address, "error": d["error"], "source": self.SERVICE_NAME}
            ctype = d.get("connectionType", "")
            return {
                "ip_address": ip_address, "source": self.SERVICE_NAME,
                "country_code": d.get("countryCode", ""), "country_name": d.get("countryName", ""),
                "city": d.get("city", ""), "state_province": d.get("stateProv", ""),
                "latitude": float(d.get("latitude", 0) or 0),
                "longitude": float(d.get("longitude", 0) or 0),
                "isp": d.get("isp", ""), "organization": d.get("organization", ""),
                "connection_type": ctype, "currency": d.get("currency", ""),
                "is_proxy": ctype in ("DCH", "SES", "PUB"),
                "is_hosting": ctype == "DCH", "is_mobile": ctype == "MOB",
                "is_residential": ctype == "RES",
            }
        except Exception as e:
