...
class TranslationService:
    ...
    def get_usage_statistics(self) -> Dict:
        ...
        if not stats:
            ...
            # FIXED: Real statistics collection from LocalizationAnalytics model
            try:
                ...
                qs = LocalizationAnalytics.objects.filter(
                    event_type__in=[
                        LocalizationAnalytics.EventType.TRANSLATION_REQUESTED,
                        LocalizationAnalytics.EventType.TRANSLATION_MISSING,
                    ],
                    created_at__gte=cutoff
                )

                today_cutoff = dj_tz.now() - timedelta(hours=24)
                today_qs = qs.filter(created_at__gte=today_cutoff)

                by_lang = list(
                    qs.values('from_language', 'to_language')
                ...
                total = qs.count()
                missing = qs.filter(
                    event_type=LocalizationAnalytics.EventType.TRANSLATION_MISSING
                ).count()
                success_rate = round(((total - missing) / total * 100), 1) if total > 0 else 100.0
                ...
                stats = {
                    ...
                    'by_language_pair': [
                        {
                            'from': pair.get('from_language', ''),
                            'to': pair.get('to_language', ''),
                            'count': pair.get('count', 0),
                        }
                        for pair in by_lang
                    ],
                    'success_rate': success_rate,
                    'average_response_time': round(float(avg_resp), 1),
                    'today_count': today_qs.count(),
                    'missing_count': missing,
                    'period_days': 30,
                }
            except Exception as stats_err:
                logger.error(f"Statistics collection failed: {stats_err}")
