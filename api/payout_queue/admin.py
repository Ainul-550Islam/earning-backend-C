...
def _force_register_payout_queue():
    try:
        from api.admin_panel.admin import admin_site as modern_site
        if modern_site is None:
            return
        pairs = [(PayoutBatch, PayoutBatchAdmin), (PayoutItem, PayoutItemAdmin), (WithdrawalPriority, WithdrawalPriorityAdmin), (BulkProcessLog, BulkProcessLogAdmin)]
        registered = 0
        for model, model_admin in pairs:
            try:
                ...
                pass
        print(f"[OK] payout_queue registered {registered} models")
    except Exception as e:
        print(f"[WARN] payout_queue: {e}")
