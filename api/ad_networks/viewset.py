"""
api/ad_networks/viewset.py
ViewSet for ad networks module
...
class OfferCategoryViewSet(viewsets.ModelViewSet):
    """
    ViewSet for OfferCategory model
    """
        ...
        try:
            ...
            return Response({
                ...
                'analytics': {
                    ...
                    'avg_payout': float(total_payout / approved_conversions) if approved_conversions > 0 else 0
                }
            }, status=status.HTTP_200_OK)
...
class UserOfferEngagementViewSet(viewsets.ModelViewSet):
    """
    ViewSet for UserOfferEngagement model
    """
...
class OfferConversionViewSet(viewsets.ModelViewSet):
    """
    ViewSet for OfferConversion model
...
class NetworkHealthCheckViewSet(viewsets.ModelViewSet):
    """
    ViewSet for NetworkHealthCheck model
    Admin access only
    """
    ...
    @action(detail=False, methods=['post'])
    def check_all_networks(self, request):
...
class OfferRecommendationsView(generics.ListAPIView):
    """
    View for personalized offer recommendations
    """
