...
# ==================== ERROR HANDLING TESTS ====================
    ...
    def test_invalid_foreign_key(self):
        """ইনভ্যালিড ফরেন কী টেস্ট"""
        from django.core.exceptions import ValidationError
        from django.db import IntegrityError
        
        # Category ছাড়া অফার তৈরি করার চেষ্টা করুন
        ad_network = create_test_ad_network()
        
        # আমরা এখন ValidationError অথবা IntegrityError যেকোনো একটি আশা করছি
        with self.assertRaises((ValidationError, IntegrityError)):
            offer = Offer(
                ad_network=ad_network,
                # এখানে category এবং description নেই, যা নিয়ম ভঙ্গ করে
                external_id='ERROR_TEST_001',
                title='Error Test Offer',
                reward_amount=Decimal('5.00'),
                reward_currency='BDT',
                click_url='https://example.com/error-test',
                status='active'
            )
            # category না থাকায় full_clean() নিচের ValidationError থ্রো করবে
...
#         status='active'
#     )
#     offer.full_clean() # এটি মডেল ভ্যালিডেশন চেক করবে
#     offer.save()       # এটি ডাটাবেস লেভেলে এরর ধরবে
        
        # with self.assertRaises(IntegrityError):
        #     Offer.objects.create(
        #         ad_network=ad_network,
        #         # category missing - should raise error
        #         external_id='ERROR_TEST_001',
        #         title='Error Test Offer',
