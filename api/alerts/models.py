# alerts/models.py - OPTIMIZED COMPLETE SYSTEM
...
class AlertSchedule(models.Model):
    ...
    # days_of_week = models.JSONField(default=[0, 1, 2, 3, 4], help_text="List of days (0=Monday)")
    days_of_week = models.JSONField(
    default=get_default_days_of_week, # সরাসরি আপনার তৈরি করা ফাংশনটির নাম দিন
...
# এই ফাংশনগুলো একদম বাম পাশ থেকে শুরু হবে, কোনো স্পেস থাকবে না
...
# alerts/models.py এর শেষে যোগ করুন
...
#         self.full_clean()
#         super().save(*args, **kwargs)
#         self.active.clear_cache()  # Clear cached active rules
...
# @receiver(pre_save, sender=AlertRule)
# def validate_alert_rule_before_save(sender, instance, **kwargs):
#     """Pre-save validation for AlertRule"""
#     try:
#         instance.full_clean()
#     except ValidationError as e:
#         logger.error(f"Validation error for AlertRule {instance.name}: {e}")
...
#     if not created and instance.pk:
#         try:
#             old_instance = AlertRule.objects.get(pk=instance.pk)
#             changed_fields = []
...
#             old_data = {
#                 field.name: _safe_serialize(getattr(old_instance, field.name))
...
#             )
#         except AlertRule.DoesNotExist:
#             logger.warning(f"AlertRule {instance.pk} not found for history logging")
