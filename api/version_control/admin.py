...
def _force_register_version_control():
    try:
        from api.admin_panel.admin import admin_site as modern_site
        if modern_site is None:
            return
        pairs = [(AppUpdatePolicy, AppUpdatePolicyAdmin), (MaintenanceSchedule, MaintenanceScheduleAdmin), (PlatformRedirect, PlatformRedirectAdmin)]
        registered = 0
        for model, model_admin in pairs:
            try:
                ...
                pass
        print(f"[OK] version_control registered {registered} models")
    except Exception as e:
        print(f"[WARN] version_control: {e}")
