...
class Notification(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['user', 'is_read'], name='idx_user_is_read_1120'),
            models.Index(fields=['user', 'status'], name='idx_user_status_1121'),
            models.Index(fields=['user', 'created_at'], name='idx_user_created_at_1122'),
            models.Index(fields=['notification_type'], name='idx_notification_type_1123'),
            models.Index(fields=['priority'], name='idx_priority_1124'),
            models.Index(fields=['status'], name='idx_status_1125'),
            models.Index(fields=['channel'], name='idx_channel_1126'),
            models.Index(fields=['scheduled_for'], name='idx_scheduled_for_1127'),
            models.Index(fields=['expire_date'], name='idx_expire_date_1128'),
            models.Index(fields=['batch_id'], name='idx_batch_id_1129'),
            models.Index(fields=['campaign_id'], name='idx_campaign_id_1130'),
            models.Index(fields=['group_id'], name='idx_group_id_1131'),
            models.Index(fields=['is_deleted'], name='idx_is_deleted_1132'),
            models.Index(fields=['language'], name='idx_language_1133'),
        ]
    ...
    def get_stats(cls, user=None, start_date=None, end_date=None):
        ...
        stats = {
            ...
            'daily_count': list(queryset.extra({
...
class NotificationTemplate(models.Model):
    ...
    class Meta:
        ...
        ordering = ['name']
        indexes = [
            models.Index(fields=['name'], name='idx_name_1134'),
            models.Index(fields=['template_type'], name='idx_template_type_1135'),
            models.Index(fields=['category'], name='idx_category_1136'),
            models.Index(fields=['is_active'], name='idx_is_active_1137'),
        ]
...
class DeviceToken(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['user', 'is_active'], name='idx_user_is_active_1138'),
            models.Index(fields=['device_type'], name='idx_device_type_1139'),
            models.Index(fields=['platform'], name='idx_platform_1140'),
            models.Index(fields=['last_active'], name='idx_last_active_1141'),
        ]
...
class NotificationCampaign(models.Model):
    """
    Campaign for sending bulk notifications
    """
    ...
    ab_test_variants = models.JSONField(
        default=list,
        blank=True
    )
    ...
    def calculate_target_count(self):
        ...
        filters = self.target_segment.get('filters', {})
        
        if filters.get('registration_date'):
            if filters['registration_date'].get('start'):
                queryset = queryset.filter(
                    date_joined__gte=filters['registration_date']['start']
                )
            if filters['registration_date'].get('end'):
                queryset = queryset.filter(
                    date_joined__lte=filters['registration_date']['end']
                )
        ...
        if filters.get('has_made_purchase'):
            # This would need custom logic based on your models
            pass
        
        self.target_count = queryset.count()
        self.save()
    ...
    def update_progress(self):
        ...
        # Get stats from notifications
        stats = Notification.objects.filter(
            batch_id=self.id
        ).aggregate(
            total_sent=Count('id'),
            total_delivered=Count('id', filter=models.Q(is_delivered=True)),
            total_failed=Count('id', filter=models.Q(status='failed')),
            total_read=Count('id', filter=models.Q(is_read=True)),
            total_clicked=Sum('click_count'),
            total_cost=Sum('cost')
        )
        
        self.total_sent = stats['total_sent'] or 0
        self.total_delivered = stats['total_delivered'] or 0
        self.total_failed = stats['total_failed'] or 0
        self.total_read = stats['total_read'] or 0
        self.total_clicked = stats['total_clicked'] or 0
        self.total_cost = stats['total_cost'] or 0
        
        # Calculate rates
        if self.total_sent > 0:
            self.delivery_rate = (self.total_delivered / self.total_sent) * 100
            self.open_rate = (self.total_read / self.total_sent) * 100
            self.click_through_rate = (self.total_clicked / self.total_sent) * 100
    ...
    def start(self):
        ...
        process_campaign_task.delay(str(self.id))
    ...
    def cancel(self):
        ...
        if self.status in ['completed', 'cancelled', 'failed']:
            raise ValidationError("Cannot cancel campaign in current status")
    ...
    def get_progress_percentage(self):
        ...
        return min(100, (self.total_sent / self.target_count) * 100)
    
    def is_completed(self):
        """Check if campaign is completed"""
        return self.status == 'completed' or self.total_sent >= self.target_count
...
class NotificationRule(models.Model):
    ...
    def execute(self, context=None):
        ...
        try:
            from .services import NotificationRuleService
...
class NotificationLog(models.Model):
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['notification'], name='idx_notification_1142'),
            models.Index(fields=['user'], name='idx_user_1143'),
            models.Index(fields=['log_type'], name='idx_log_type_1144'),
            models.Index(fields=['log_level'], name='idx_log_level_1145'),
            models.Index(fields=['created_at'], name='idx_created_at_1146'),
        ]
...
class Notice(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['status', 'is_published'], name='idx_status_is_published_1147'),
            models.Index(fields=['notice_type'], name='idx_notice_type_1148'),
            models.Index(fields=['priority'], name='idx_priority_1149'),
            models.Index(fields=['start_date', 'end_date'], name='idx_start_date_end_date_1150'),
            models.Index(fields=['is_pinned'], name='idx_is_pinned_1151'),
            models.Index(fields=['created_at'], name='idx_created_at_1152'),
        ]
